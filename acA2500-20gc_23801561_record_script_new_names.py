...
import sys, time, datetime, os, cv2
#Recording Function
    ...
    while True:
        try:
            ...
            if grabResult.GrabSucceeded():
                ...
                WRITER.append_data(cv2.cvtColor(grabResult.Array,cv2.COLOR_BAYER_RG2RGB))
                grabResult.Release()
                acquired_images += 1
...
def Find_my_cam_by_IP():
    ...
    camera.Open()
    camera_name=str(camera.GetDeviceInfo().GetSerialNumber())
    create_camera_folder(camera_name)
    nodeFile=camera_name+"/"+str(camera.GetDeviceInfo().GetModelName())+"_"+str(camera.GetDeviceInfo().GetSerialNumber())+".pfs"
...
def create_files_and_writers(device):
    grab_file_path={}
    camera_name=str(device.GetDeviceInfo().GetSerialNumber())
    file_name =camera_name+"_"+str(datetime.datetime.now())
    for t in [" ","-"]:
        file_name=file_name.replace(t,"_")
    for t in [":","."]:
        file_name=file_name.replace(t,"x")
    for t in [":"," "]:
...
full_day=2851200
record_cam(camera,writer,5000,start_recording,grab_file_path)
writer.close()
#camera.Close()
